...
def refined_text(query_result, prompt):
    try:
        prompt = f"""
            You are a great financial data analyist
            You have been provided with the output for a particular prompt and you have
            been provided the query prompt.
            Draw insights from the query_result and then give some recommendation along with 
            a good way to give the result back to the user
            Output format should be a string
            prompt is -> {prompt}
            query_result -> {json.dumps(query_result, default=str)}

            return the result as string and there should not be end line characters in it like /n
         """
        print("Sending request to Gemini model for AGENT...")
        ...
        print(f"Error in fetching infoooooooo: {e}")
        return e
...
def agent_task(json):
    ...
    try:
        ...
        * Here are some restrictions on the query:
        ...
        # Getting human response data
        response_data = refined_text(query_result, json['prompt'])
